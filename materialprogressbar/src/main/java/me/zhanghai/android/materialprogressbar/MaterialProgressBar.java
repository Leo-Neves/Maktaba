...
public class MaterialProgressBar extends ProgressBar {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        color = a.getColor(R.styleable.MaterialProgressBar_progressColor, Color.parseColor("#009486"));
        ...
        switch (mProgressStyle) {
            case PROGRESS_STYLE_CIRCULAR:
                if (!isIndeterminate() || setBothDrawables) {
                    ...
                    setIndeterminateDrawable(new IndeterminateProgressDrawable(context, color));
                }
                break;
            case PROGRESS_STYLE_HORIZONTAL:
                if (isIndeterminate() || setBothDrawables) {
                    setIndeterminateDrawable(new IndeterminateHorizontalProgressDrawable(context, color));
                }
                if (!isIndeterminate() || setBothDrawables) {
                    setProgressDrawable(new HorizontalProgressDrawable(context, color));
                }
                break;
            default:
