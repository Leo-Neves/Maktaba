...
class ArcMetric {
    ...
    private void calcRadius() {
        ...
        mRadius = mStartEndSegment / sin(mAnimationDegree) * sin(mSideDegree);
    }

    private void calcMidAxisSeg() {
        mMidAxisSegment = mRadius * sin(mSideDegree);
    }
    ...
    private void calcDegrees() {
        ...
        mZeroStartDegree = acos((2 * Math.pow(mRadius, 2) - Math.pow(mZeroStartSegment, 2)) / (2 * Math.pow(mRadius, 2)));
        switch (mSide) {
            case RIGHT:
