...
public class BottomBar extends FrameLayout {
    ...
    private @ColorInt
    int itemActiveColor, itemInactiveColor;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs!=null){
            ...
            gravity = a.getResourceId(R.styleable.BottomBar_android_gravity, Gravity.CENTER);
        }
    }
    ...
    /**
     * Remove an item at the given tag
     */
    public void removeItemAtTag(Object tag) {
        int index = -1;
        if (tag == null) {
            Log.w(TAG, "The position is null and cannot be used to find item");
            index = -1;
        }
        for (int i=0; i<items.size(); i++) {
            BottomBarNavigationItem item = items.get(i);
            if (item.getTag() != null && tag.equals(item.getTag()))
                index = i;
        }
        if (index > -1) {
    ...
    /**
     * Set the gravity of itens (default is Gravity.CENTER)
     *
    ...
    /**
     * Get item at the given tag
     *
     * @param tag Object: item tag
     * @return The item at the given tag
     * @requisito R01
     */
    public BottomBarNavigationItem getItemByTag(Object tag) {
        if (tag == null) {
            Log.w(TAG, "The position is null and cannot be used to find item");
            return null;
        }
        for (BottomBarNavigationItem item : items) {
            if (item.getTag() != null && tag.equals(item.getTag()))
                return item;
        }
        return null;
